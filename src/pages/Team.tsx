...
// Agent-OS tool-execution approvals — a genuinely different queue from the
// content-publish one above (that's "should this upload go out," this is
// "should this agent be allowed to run this command"). Shown as its own
// panel rather than merged into ApprovalsTab's list, per the architecture
// plan's "don't duplicate systems, but don't conflate different concepts
// either" guidance.
function AgentOsApprovalsPanel() {
  const { connection, approvals, busyId, error, act } = useAgentOsApprovals()

  if (connection === 'unconfigured') return null

  return (
    <Panel title="Agent-OS Approvals" code={connection === 'live' ? 'LIVE' : connection.toUpperCase()} accent="#58b6f0">
      {connection === 'error' && <p className="p-1 text-xs text-danger">Agent-OS gateway unreachable{error ? `: ${error}` : ''}.</p>}
      {connection === 'connecting' && <p className="p-1 text-xs text-dim">Connecting…</p>}
      {connection === 'live' && (
        <>
          {error && <p className="mb-2 border border-danger/40 bg-danger/10 p-2 text-xs text-danger">{error}</p>}
          {!approvals.length && (
            <p className="p-1 text-xs text-dim">No tool-execution approvals pending. These appear when an agent's permission policy requires a human to say yes before a command runs.</p>
          )}
