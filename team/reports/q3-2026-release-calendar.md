...
Status: art + master done. Video pending — that is the only blocker.
Homerun is ready except for a video asset. Three weeks after Switch gives each release
its own promotional moment. August 8 lands on a Saturday, which tends to perform well
for melodic/electronic releases (audience is online, editorial playlists often update
Friday–Saturday).

Blockers:
- Video must be produced and delivered before approximately July 25 to allow time for
  upload, review, and scheduling.
- If video slips past August 1, the release moves to August 22.

Pre-release tasks:
- User/Nyx: confirm video production path (internal edit vs. commissioned)
- Nyx: build upload package once video is confirmed
- Hemera: re-confirm date once video ETA is known (by July 1 check-in)
...
3. No video production pipeline is confirmed for Homerun. This needs a decision by
   July 1 to protect the August 8 date.
